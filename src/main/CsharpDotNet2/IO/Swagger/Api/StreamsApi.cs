...
namespace IO.Swagger.Api
{
 ...
 if (keyByType != null) queryParams.Add("key_by_type", ApiClient.ParameterToString(keyByType)); // query parameter
        ...
        public StreamSet GetRouteStreams (long? id)
        {
            ...
            // verify the required parameter 'id' is set
            if (id == null) throw new ApiException(400, "Missing required parameter 'id' when calling GetRouteStreams");
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling GetRouteStreams: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling GetRouteStreams: " + response.ErrorMessage, response.ErrorMessage);
